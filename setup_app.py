...
DB_NAME = os.environ.get('AHERNP_DATABASE_NAME')
DB_USER = os.environ.get('AHERNP_DATABASE_USER')
DB_PASSWORD = os.environ.get('AHERNP_DATABASE_PASSWORD')
...
def setup_postgres():
    ...
    commands = [
        ...
        'CREATE USER {db_user} WITH PASSWORD \'{db_password}\';'.format(db_user=DB_USER, db_password=DB_PASSWORD),
        'ALTER ROLE {db_user} SET client_encoding TO \'utf8\';'.format(db_user=DB_USER),
        'ALTER ROLE {db_user} SET default_transaction_isolation TO \'read committed\';'.format(db_user=DB_USER),
        'ALTER ROLE {db_user} SET timezone TO \'UTC\';'.format(db_user=DB_USER),
        'GRANT ALL PRIVILEGES ON DATABASE {db_name} TO {db_user};'.format(db_name=DB_NAME, db_user=DB_USER),
